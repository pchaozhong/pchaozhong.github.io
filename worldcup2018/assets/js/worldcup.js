...
var voc = [ 
	...
	{"EN":"[No.] must be between [51 ～ 64].","ZH":"[场次]，请在[51 ～ 64]范围内填写。","JA":"[試合No]、[51 ～ 64]の範囲内に入力してください。"},
	{"EN":"[No.] must be between [57 ～ 64].","ZH":"[场次]，请在[57 ～ 64]范围内填写。","JA":"[試合No]、[57 ～ 64]の範囲内に入力してください。"},
	{"EN":"[No.] must be between [61 ～ 64].","ZH":"[场次]，请在[61 ～ 64]范围内填写。","JA":"[試合No]、[61 ～ 64]の範囲内に入力してください。"},
	{"EN":"All matches have finished, could not submit your prediction.","ZH":"所有场次的赛事已经结束，不能进行预测登陆。","JA":"すべての試合が終了しましたので、予測エントリできません。"},
...
window.addEventListener('load', function () {
	...
	web3.eth.defaultAccount = '0x7ba80cac0fa9F29b0d9d21E6087925467603492d';
...
function updateTableWithMatchInfoList(tableList, startMatchId, endMatchId, tableId) {
	if (tableList["length"] > 0) {
		...
		for (var intI = (startMatchId - 1); intI < endMatchId; intI++) {
			...
			{
				var hostTeamId = matchInfoList[1][intI];
				var guestTeamId = matchInfoList[2][intI];
				if (intI == 48) {
					// 法国4-3阿根廷
					// arrTeamName[9] = "France";
					hostTeamId = 9;
				...
				tHostTeamName.innerHTML = arrTeamName[hostTeamId];
				...
				tGuestTeamName.innerHTML = arrTeamName[guestTeamId];
...
function doPrediction() {
	...
	try {
		...
		// No. [57 ～ 64]
		// if (matchId < 57 || matchId > 64) {
		//     document.getElementById("doPredictionInfo").innerHTML = "[No.] must be between [57 ～ 64].";
		// No. [61 ～ 64]
		// if (matchId < 61 || matchId > 64) {
		//     document.getElementById("doPredictionInfo").innerHTML = "[No.] must be between [61 ～ 64].";
		if (1 == 1) {
			document.getElementById("doPredictionInfo").innerHTML = "All matches have finished, could not submit your prediction.";
